...
function shrinkwrap (args, silent, cb) {
  ...
  move(
    ...
    readPackageTree(npm.localPrefix, andRecalculateMetadata(iferr(cb, function (tree) {
      createShrinkwrap(tree, silent, cb)
    })))
  })
}
...
function createShrinkwrap (tree, silent, cb) {
  lifecycle(tree.pkg, 'preshrinkwrap', tree.path, function () {
    var pkginfo = treeToShrinkwrap(tree)

    chain([
      [lifecycle, tree.package, 'shrinkwrap', tree.path],
      [shrinkwrap_, tree.path, pkginfo, silent],
...
function shrinkwrapDeps (problems, deps, top, tree, seen) {
  ...
  tree.children.sort(function (aa, bb) { return moduleName(aa).localeCompare(moduleName(bb)) }).forEach(function (child) {
    ...
    var req = child.package._requested
...
function shrinkwrap_ (dir, pkginfo, silent, cb) {
  if (pkginfo.problems) {
  ...
  save(dir, pkginfo, silent, cb)
}

function save (dir, pkginfo, silent, cb) {
  ...
  BB.join(
    checkShrinkwrap(dir, SHRINKWRAP),
    checkShrinkwrap(dir, PKGLOCK),
    (shrinkwrap, lockfile) => {
      const file = (
        shrinkwrap ||
        lockfile ||
        path.resolve(dir, PKGLOCK)
      )
...
function checkShrinkwrap (dir, name) {
